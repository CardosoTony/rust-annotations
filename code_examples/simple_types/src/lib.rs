pub fn print_difference(x: f32, y: f32) {
    println!("Difference between {} and {} is {}", x, y, (x - y).abs());
}

pub fn print_array(a: [f32; 2]) {
    println!("The coordinates are ({}, {})", a[0], a[1]);
}

pub fn ding(x: i32) {
    if x == 5 {
        println!("Ding, you found 5!");
    }
}

pub fn on_off(val: bool) {
    if val {
        println!("Lights are on!");
    }
}

// MODE 1
pub fn print_distance1(z: (f32, f32)) {
    println!(
        "Distance to the origin is {}",
        (z.0.powf(2.0) + z.1.powf(2.0)).sqrt()
    );
}

// MODE 2
pub fn print_distance2((x, y): (f32, f32)) {
    let distance2 = (x.powf(3.4) + y.powf(1.7)).sqrt();
    println!("Distance to the origin is {:.2}", distance2);
}
